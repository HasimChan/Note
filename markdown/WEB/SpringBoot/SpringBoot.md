...
> 方便使用的背后是SpringBoot帮我们做了很多的工作，比如很多的依赖我们不需要管理和配置，通过引入starter就能完成
...
```xml
1、见到很多 spring-boot-starter-* ： *代表某种场景，比如web
2、只要引入starter，这个场景的所有常规需要的依赖我们都自动引入
3、SpringBoot所有支持的场景见以下链接
...
- - 主程序所在包及其下面的所有子包里面的组件都会被默认扫描进来
  - 无需以前的包扫描配置
  - 想要改变扫描路径，@SpringBootApplication(scanBasePackages=**"com.hasim"**)
...
>
> 1. 注解配置导入
>    1. 通过配置类添加，声明的配置类本身也是一个组件，需要注意配置类是否声明为单例
>    2. 普通组件的声明，同Spring框架
>    3. 将类装配进容器，更改扫描包路径
>    4. 以一定的条件对类中的组件进行装配
> 2. 配置文件配置导入
>    1. 引入配置文件
> 3. 将配置文件中的配置读取到 JavaBean
...
### 2、@Bean、@Component、@Controller、@Service、@Repository 组件声明



### 3、@ComponentScan、@Import 组件导入
...
========================== 测试条件装配 ==========================
@Configuration(proxyBeanMethods = false) //告诉SpringBoot这是一个配置类 == 配置文件
//@ConditionalOnBean(name = "tom")  // 如果容器中存在名为 tom 的组件则加载此配置文件配置的组件
@ConditionalOnMissingBean(name = "tom") // 与上面相反
public class MyConfig {
...
/**
 * @Component 将此类装配到容器中
 * @ConfigurationProperties 指定配置绑定的配置文件中的前缀名
 	mycar.brand=BYDhh
 	mycar.price=1000000
...
// Car类上需注明：@ConfigurationProperties(prefix = "mycar")
```

# 3、自动配置原理入门

> SpringBoot 怎么帮我们实现自动配置？
>
> 
...
> 完成 MainApplication 所在包下所有组件的导入
...
![yuque_diagram](SpringBoot.assets/yuque_diagram-1623133541344.jpg)
