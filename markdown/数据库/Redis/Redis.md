...
>
> [Redis入门教程](https://blog.csdn.net/liqingtx/article/details/60330555?ops_request_misc=%7B%22request%5Fid%22%3A%22162380099116780357299511%22%2C%22scm%22%3A%2220140713.130102334..%22%7D&request_id=162380099116780357299511&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_positive~default-2-60330555.first_rank_v2_pc_rank_v29&utm_term=redis&spm=1018.2226.3001.4187)
>
> [Linux安装redis详细教程](https://blog.csdn.net/u011159417/article/details/80085011?utm_medium=distribute.pc_relevant.none-task-blog-2~default~BlogCommendFromMachineLearnPai2~default-1.control&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2~default~BlogCommendFromMachineLearnPai2~default-1.control)
>
> [Redis面试题（2020最新版）](https://blog.csdn.net/ThinkWon/article/details/103522351?ops_request_misc=%7B%22request%5Fid%22%3A%22162380099116780357299511%22%2C%22scm%22%3A%2220140713.130102334..%22%7D&request_id=162380099116780357299511&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_click~default-6-103522351.first_rank_v2_pc_rank_v29&utm_term=redis&spm=1018.2226.3001.4187)
...
* 在指定的时间间隔内将内存中的数据集快照写入磁盘，。快照文件默认被存储在当前文件夹中，名称为`dump.rdb`，可以通过dir和dbfilename参数（\### SNAPSHOTTING ###）来修改默认值
...
# redis是基于内存的数据库，可以通过设置该值定期写入磁盘。
# 注释掉“save”这一行配置项就可以让保存数据库功能失效
# 900秒（15分钟）内至少1个key值改变（则进行数据库保存--持久化）到点才持久化
# 300秒（5分钟）内至少10个key值改变（则进行数据库保存--持久化） 
# 60秒（1分钟）内至少10000个key值改变（则进行数据库保存--持久化）
save 900 1
save 300 10
...
2. 手动通过save和bgsave命令
   ...
   * save：save时只管保存，其他不管，全部**阻塞**
   * bgsave：redis会在后台**异步**的进行快照操作，同时还可以响应客户端请求。可以通过lastsave命令获取最后一次成功执行快照的事件

3. 执行 flushall 命令，也会产生 dump.rdb 文件，但是里面是空的，无意义。
4. 执行 shutdown 命令，安全退出，也会生成快照文件（和异常退出形成对比，比如：kill杀死进程的方式）

#### 3. 如何恢复数据

将备份文件 (dump.rdb) 移动到 dir 目录并启动服务即可（CONFIG GET dir获取目录）

**如何停止RDB保存规则：** `redis-cli config set save ""`

#### 4. 优缺点

* **优势**
  1. 恢复数据的速度很快，适合大规模的数据恢复，而对数据完整性和一致性要求不高的情况
...
![image-20210617221037831](Redis.assets/image-20210617221037831.png)
...
* aof 机制默认关闭，可以通过`appendonly = yes`参数开启 aof 机制（优先于 RDB）
* 通过`appendfilename = myaoffile.aof`指定aof文件名称
...
1. 是什么
   ...
   * 当AOF文件的大小超过所设定的阈值时，Redis就会启动AOF文件的内容压缩，只保留可以恢复数据的最小指令集，可以使用命令 bgrewriteaof 执行重写
2. 重写原理
...
aop重写和aop追加同时发生的问题，可以通过`no-appendfsync-on-rewrite no`进行配置
...
![image-20210617223625287](Redis.assets/image-20210617223625287.png)
...
5. 性能建议：
   ...
   >
   > 如果不Enable AOF ，仅靠Master-Slave Replication 实现高可用性也可以。能省掉一大笔IO也减少了rewrite时带来的系统波动。代价是如果Master/Slave同时down掉，会丢失十几分钟的数据，启动脚本也要比较两个Master/Slave中的RDB文件，载入较新的那个。新浪微博就选用了这种架构
...
| unwatch         | 取消watch命令对**所有** key 的监视                           |

#### 2. 常见 Case

1. **正常执行**

   ![](Redis.assets/20200329153535143.png)

2. **放弃事务**

   ![](Redis.assets/20200329153617123.png)

3. **全体连坐（出现错误）**

   ![image-20210617225335705](Redis.assets/image-20210617225335705.png)

4. <span id="yuan"><b>冤头债主</b></span>

![](Redis.assets/20200329153742186.png)
...
2. 案例
   ...
   > 1. 初始化信用卡可用余额和欠额
   >
   > ![image-20210617225728744](Redis.assets/image-20210617225728744.png)
   >
   > 2. 无加塞篡改，先监控再开启multi，保证两笔金额变动在同一个事务内
   >
   >    ![image-20210617225814755](Redis.assets/image-20210617225814755.png)
   >
   > 3. 有加塞篡改（监控了key，如果key被修改了，后面一个事务的执行失效）
   >
   >    ![image-20210617225828958](Redis.assets/image-20210617225828958-1623941958432.png)
   >
   > 4.  unwatch
   >
   >    ![image-20210617225910244](Redis.assets/image-20210617225910244-1623941953793.png)
   >
   > 5. 一旦执行了exec 或 discard，之前加的监控锁都会被取消掉了
...
3. 不保证原子性：redis同一个事务中如果有一条命令执行失败，其后的命令仍然会被执行，没有回滚，参考[冤头债主]
...
## 5. Redis主从复制（Master/Slave）

### 5.1 是什么

* 行话：也就是我们所说的主从复制，主机数据更新后根据配置和策略，自动同步到备机的 **master/slaver机制**
...
3. 修改配置文件细节操作
   ...
   * 拷贝多个redis.conf文件
     ...
     ![image-20210617231823656](Redis.assets/image-20210617231823656.png)

   * 开启daemonize yes

     ![image-20210617231848148](Redis.assets/image-20210617231848148.png)
   ...
   * Log文件名字
     ...
     ![image-20210617231917708](Redis.assets/image-20210617231917708-1623943159502.png)

   * Dump.rdb名字

     ![image-20210617231933577](Redis.assets/image-20210617231933577.png)

#### 2. 常用3招

1. **一主二仆**（就是一个Master两个Slave）

![image-20210617232213857](Redis.assets/image-20210617232213857.png)

**问题：**

1. 切入点问题？slave1、slave2是从头开始复制还是从切入点开始复制?比如从k4进来，那之前的123是否也可以复制？

   第一次slave1 和slave2 切入点，是全量复制（从头开始复制），之后是增量复制
...
2. **薪火相传**（链表）
   * 上一个Slave可以是下一个slave的Master，Slave同样可以接收其他 slaves 的连接和同步请求，那么该slave作为了链条中下一个的master,
     可以有效减轻 master 的写压力
...
![image-20210617233615667](Redis.assets/image-20210617233615667.png)
